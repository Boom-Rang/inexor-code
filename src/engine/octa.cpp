...
int getmippedtexture(const cube &p, int orient)
{
    ...
    for(int i = (numtexs)-1; i >= 0; i--) if(!i || texs[i] > DEFAULT_SKY) return texs[i];
    return DEFAULT_GEOM;
}
...
static void genedgespanvert(ivec &p, cube &c, vec &v)
{
    ...
    //assert(v.y>=0 && v.y<=8);
    //assert(v.z>=0 && v.z<=8);
    v.x = std::max(0.0f, std::min(8.0f, v.x));
    v.y = std::max(0.0f, std::min(8.0f, v.y));
    v.z = std::max(0.0f, std::min(8.0f, v.z));
}
...
void genmerges(cube *c = worldroot, const ivec &o = ivec(0, 0, 0), int size = worldsize>>1)
{
    ...
    for(int i = 0; i < int(8); i++)
    {
        ...
        else if(!isempty(c[i])) for(int j = 0; j < int(6); j++) if((vis = visibletris(c[i], j, co.x, co.y, co.z, size)))
        {
            cfkey k;
            poly p;
