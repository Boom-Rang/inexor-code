...
ICOMMAND(min, "V", (tagval *args, int numargs),
{
    ...
    for(int i = 0; i < int(numargs - 1); i++) val = std::min(val, args[i].getint());
    intret(val);
});
ICOMMAND(max, "V", (tagval *args, int numargs),
{
    int val = numargs > 0 ? args[numargs - 1].getint() : 0;
    for(int i = 0; i < int(numargs - 1); i++) val = std::max(val, args[i].getint());
    intret(val);
});
ICOMMAND(minf, "V", (tagval *args, int numargs),
{
    float val = numargs > 0 ? args[numargs - 1].getfloat() : 0.0f;
    for(int i = 0; i < int(numargs - 1); i++) val = std::min(val, args[i].getfloat());
    floatret(val);
});
ICOMMAND(maxf, "V", (tagval *args, int numargs),
{
    float val = numargs > 0 ? args[numargs - 1].getfloat() : 0.0f;
    for(int i = 0; i < int(numargs - 1); i++) val = std::max(val, args[i].getfloat());
    floatret(val);
});
