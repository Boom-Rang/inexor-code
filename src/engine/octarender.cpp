...
struct vacollect : verthash
{
    ...
    void remapunlit(vector<sortkey> &remap)
    {
        ...
        for(int j = 0; j < int(2); j++)
        {
            ...
            for(int i = 0; i < int(std::max(firstlit[offset], firstlit[offset+1])); i++)
            {
                sortkey &k = texs[i];
...
void addgrasstri(int face, vertex *verts, int numv, ushort texture, ushort lmid)
{
    ...
    g.center.div(numv);
    g.radius = 0;
    for(int k = 0; k < int(numv); k++) g.radius = std::max(g.radius, g.v[k].dist(g.center));
...
void addcubeverts(VSlot &vslot, int orient, int size, vec *pos, int convex, ushort texture, ushort lmid, vertinfo *vinfo, int numverts, int tj = -1, ushort envmap = EMID_NONE, int grassy = 0, bool alpha = false, int layer = LAYER_TOP)
{
    ...
    {
        for(int k = 0; k < int(numverts); k++) vc.skyclip = std::min(vc.skyclip, int(pos[k].z*8)>>3);
...
void gencubeedges(cube &c, int x, int y, int z, int size)
{
    ...
    int vis;
    for(int i = 0; i < int(6); i++) if((vis = visibletris(c, i, x, y, z, size)))
    {
...
void gencubeverts(cube &c, int x, int y, int z, int size, int csi)
{
    ...
    for(int i = 0; i < int(6); i++) if(vismask&(1<<i) && (vis = visibletris(c, i, x, y, z, size)))
    {
        ...
        if(numverts)
        {
            ...
            for(int j = 0; j < int(numverts); j++) pos[j] = verts[j].getxyz().tovec().mul(1.0f/8).add(vo);
