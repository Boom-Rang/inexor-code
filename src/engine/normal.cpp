...
void addnormals(cube &c, const ivec &o, int size)
{
    ...
    if(c.children)
    {
        progress++;
        size >>= 1;
        for(int i = 0; i < int(8); i++) addnormals(c.children[i], ivec(i, o.x, o.y, o.z, size), size);
        return;
    }
    ...
    for(int i = 0; i < int(6); i++) if((vis = visibletris(c, i, o.x, o.y, o.z, size)))
    {
        CHECK_CALCLIGHT_PROGRESS(return, show_addnormals_progress);
...
void calcnormals(bool lerptjoints)
{
    ...
    progress = 1;
    for(int i = 0; i < int(8); i++) addnormals(worldroot[i], ivec(i, 0, 0, 0, worldsize/2), worldsize/2);
}
